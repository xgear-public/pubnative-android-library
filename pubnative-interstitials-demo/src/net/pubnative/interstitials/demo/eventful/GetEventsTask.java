...
public class GetEventsTask extends SimpleAsyncTask<List<Event>> {
	...
	public GetEventsTask(Context ctx,
			AsyncTaskResultListener<List<Event>> resultListener) {
	...
	protected List<Event> onExecute() throws Exception {
		...
		JSONObject obj = new RESTClient2(getContext()).getJSONObject(builder
				.toString());
		JSONArray arr = obj.getJSONObject("events").getJSONArray("event");
		List<Event> list = new JSONSerializer<Event>(Event.class, getContext())
				.deserializeAll(arr);
		return list;
	}
}
